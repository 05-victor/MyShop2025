...
public partial class AdminAgentRequestsViewModel : PagedViewModelBase<AgentRequestItem>
{
    ...
    protected override async Task LoadPageAsync()
    {
        try
        {
            ...
            {
                // Determine if rejection reason should be shown (only for rejected requests)
                var isRejected = r.Status.Equals("REJECTED", StringComparison.OrdinalIgnoreCase);
                var rejectionReason = isRejected && !string.IsNullOrEmpty(r.Notes) ? r.Notes : string.Empty;
                ...
                Items.Add(new AgentRequestItem
                {
                    ...
                    Experience = r.Experience ?? string.Empty,
                    Reason = r.Reason ?? string.Empty,
                    Status = r.Status,
                    IsPending = r.Status.Equals("PENDING", StringComparison.OrdinalIgnoreCase) ? Visibility.Visible : Visibility.Collapsed,
                    RejectionReason = rejectionReason,
                    HasRejectionReason = !string.IsNullOrEmpty(rejectionReason) ? Visibility.Visible : Visibility.Collapsed
                });
            }
    ...
    private async Task RejectRequest(AgentRequestItem request)
    {
        try
        {
            ...
            var rejectDialog = new Views.Dialogs.RejectReasonDialog();
            rejectDialog.XamlRoot = App.MainWindow?.Content?.XamlRoot;

            var dialogResult = await rejectDialog.ShowAsync();
            ...
            var rejectionReason = rejectDialog.RejectionReason;
            if (string.IsNullOrWhiteSpace(rejectionReason))
            {
                await _toastHelper.ShowError("Rejection reason is required");
                return;
            }

            SetLoadingState(true);

            var result = await _agentRequestFacade.RejectRequestAsync(Guid.Parse(request.Id), rejectionReason);
